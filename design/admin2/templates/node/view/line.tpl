...
<a href={$node.url_alias|ezurl} class="nodeicon" onclick="ezpopmenu_showTopLevel( event, 'SubitemsContextMenu', ez_createAArray( new Array( '%nodeID%', {$:node.node_id}, '%objectID%', {$:node.object.id}, '%languages%', {$:node.object.language_js_array}, '%classList%', {$can_create_classes} ) ) , '{$:node.object.name|shorten(18)|wash(javascript)}', {$:node.node_id}, {$disable_createHere_menu}  ); return false;">{$node.class_identifier|class_icon( small, 'Click on the icon to display a context-sensitive menu.'|i18n( 'design/admin/node/view/line' ) )}</a>&nbsp;
{if $node_url}<a href={$node_url|ezurl} title="{'Node ID: %node_id Visibility: %node_visibility'|i18n( 'design/admin/node/view/line',, hash( '%node_id', $node.node_id, '%node_visibility', $node.hidden_status_string ) )}">{/if}{$node_name|wash}{if $node_url}</a>{/if}{/default}
